...
static int is_event_device(const struct dirent *dir) {
	return strncmp(HIDRAW_DEV_NAME, dir->d_name, 5) == 0;
}
...
int enum_devices() {
  ...
  struct device_info *device_list = find_hidraw_devices();
...
struct device_info *find_hidraw_devices() {
  ...
  ndev = scandir(DEV, &namelist, is_event_device, alphasort);
	if (ndev <= 0)
		return info;
